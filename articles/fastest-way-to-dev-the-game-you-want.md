...
イベントシーンの演技については、知見がありません

特にアクションについては自分が制作するのが VR 剣戟アクションであるため、そちらのアクションに習熟するのが上達につながると考えます。
...
- 脚本（ストーリー）の執筆：これからストーリーを作るため、最も優先順位が高いです。テレビで映画を見て、紙のノートに心を動かされた部分や考えたことを書く
- アート（アニメーション）制作：演技については、映画を見て研究をする。アクションについては、まず西洋剣術の本を読む。その後に思い通りの動きができるように、カメラで動きを撮影してトレーニングを行う
- カメラワーク・ライティング：まず技術書を読みます。また、VR は 1 人称なので、1 人称のグレイブエンカウンターズのような映画を見て紙のノートに考えたことを書く
- サウンド（ボイス）制作：現在開発中の作品ではボイスを入れないため、まだ優先順位は低い。声の演技の練習を行う
- アート（モデル）制作：現在開発中の作品では独自のモデルを作成しないため、まだ優先順位は低い。デッサンの練習などを行う
